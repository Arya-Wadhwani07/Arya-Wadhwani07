<h1 align="center">Arya Jay Wadhwani</h1>
<h3 align="center">Software Engineering Intern Candidate | MSCS @ USC | Backend • Distributed Systems • ML</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/arya-jay-wadhwani">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-blue?style=flat&logo=linkedin" />
  </a>
  <a href="https://github.com/Arya-Wadhwani07">
    <img src="https://img.shields.io/badge/GitHub-Portfolio-black?style=flat&logo=github" />
  </a>
  <a href="https://drive.google.com/file/d/1Q3Qyzfgo7Pyjz_UOZ7m8xb-u9OSF_2Ds/view?usp=share_link">
    <img src="https://img.shields.io/badge/Resume-Download-green?style=flat&logo=google-drive" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=Arya-Wadhwani07&label=Profile%20Views&color=0e75b6&style=flat" />
</p>

---

## 👨‍💻 About Me

🎓 **MS in Computer Science @ University of Southern California**
🎓 **B.Tech CSE @ VIT Vellore (CGPA: 9.58/10)**

I'm a **Software Engineering intern candidate** with **production experience at Hewlett Packard Enterprise**, where I worked on **large-scale storage systems, backend automation, and real-time monitoring dashboards**.

I enjoy building **reliable, scalable systems**, optimizing **backend performance**, and working across **distributed systems, ML-backed services, and full-stack platforms**.

📌 Actively applying for **Summer 2026 Software Engineering Internships (USA)**

---

## 🧠 Core Engineering Skills

### Programming Languages
<p>
  <img src="https://skillicons.dev/icons?i=cpp,python,java,go,js,ts,sql" />
</p>

### Backend & Systems
<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,django,flask" />
</p>

### Databases & Cloud
<p>
  <img src="https://skillicons.dev/icons?i=mysql,postgres,mongodb,graphql,aws,azure" />
</p>

### ML / Data
<p>
  <img src="https://skillicons.dev/icons?i=tensorflow,pytorch" />
</p>

### Tools
<p>
  <img src="https://skillicons.dev/icons?i=linux,git,github,figma" />
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Arya-Wadhwani07&show_icons=true&count_private=true&include_all_commits=true&theme=github_dark&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Arya-Wadhwani07&theme=github-dark-blue&hide_border=true" />
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Arya-Wadhwani07&layout=compact&theme=github_dark&hide_border=true" />
</p>

---

## 📅 Commit History

<p align="center">
  <img src="https://ghchart.rshah.org/Arya-Wadhwani07" alt="Arya's Commit History Chart" />
</p>

---

## 🔥 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Arya-Wadhwani07&theme=github-compact&hide_border=true&area=true&custom_title=Contribution%20History%20(All%20Time)" />
</p>

---

## 🚀 Selected Engineering Projects

### 🆘 **Sahayata — Emergency Response Platform**
- Full-stack system for prioritizing emergency radio requests
- Achieved **95% NLP classification accuracy**
- Designed REST APIs with sub-second response times
- **Tech:** React, Flask, MySQL, NLP, Node.js

---

### 🫀 **Life-E-Line — Distributed Healthcare Platform**
- Connected **500+ hospitals**
- Implemented **async request handling (1000+ req/sec)**
- Improved transparency in organ allocation workflows
- **Tech:** Node.js, MongoDB, Express, React

---

### 🎧 **LivStory — ML-Powered Content System**
- Built NLP pipeline using **TextRank**
- Generated **550+ sound effects** with **92% accuracy**
- Deployed ML inference via Flask APIs
- **Tech:** Python, TensorFlow, NLP, Flask

---

## 🏢 Industry Experience Snapshot

### **Hewlett Packard Enterprise — Software / Systems Engineer**
- Built **Python automation** reducing hardware replacement runtime by **50%**
- Developed backend APIs & dashboards for **6PB+ distributed storage clusters**
- Led development of an **LLM-powered chatbot** with < **0.5s latency**
- Worked on **integration testing, monitoring, and scalable systems**

---

## 📚 Coursework & CS Foundations

- Data Structures & Algorithms
- Analysis of Algorithms
- Operating Systems
- Database Systems
- Computer Networks

---

## 🎯 What I'm Looking For

- **Software Engineering Intern (Backend / Systems / Full-Stack)**
- Teams working on **scalable infrastructure, data pipelines, ML systems**
- Opportunities to write **production-grade code** and learn from senior engineers

---

## 🤝 Let's Connect

📧 **Email:** arya.wadhwani@gmail.com
💼 **LinkedIn:** https://www.linkedin.com/in/arya-jay-wadhwani
📄 **Resume:** https://drive.google.com/file/d/1Q3Qyzfgo7Pyjz_UOZ7m8xb-u9OSF_2Ds/view?usp=share_link
💻 **GitHub:** https://github.com/Arya-Wadhwani07

⭐ *If you find my work interesting, feel free to explore my repositories or reach out!*
